...
var RibbonBridge = function(protobufObj) {
    ...
    var _socket = {};
    // Initialize protobuf
    var builder = pb.loadProtoFile('proto/rpc.proto');
    var barobo = builder.build('barobo');
    ...
    this.connect = function(uri, callback) {
        ...
        console.log('ribbon-bridge using uri:', uri);
        _socket = new WebSocketClient(uri);
        _socket.binaryType = "arraybuffer";
        _socket.onopen = function() {
            function handshake() {
                // Send ribbon-bridge connect message
                var connect_msg = new barobo.rpc.ClientMessage({
                    'id':_msgId,
                        'request': {
        ...
        _socket.onmessage = function(message) {
            console.log('ribbon-bridge received message', message);
            var serverMessage = barobo.rpc.ServerMessage.decode(message.data);
            if(
                ...
                ) 
            {
                if(serverMessage.reply.type == barobo.rpc.Reply.Type.RESULT) {
                    if (_replyHandlers[serverMessage.inReplyTo].name !== null) {
                        ...
                        var result = _proxyProtoBuf[name].Result;
                        var result_obj = result.decode(serverMessage.reply.result.payload);
    ...
    this.fire = function(rpcName, payload, callback) {
        ...
        var fire_msg = new barobo.rpc.ClientMessage({
            'id':_msgId,
            'request': {
